...
router.post("/referti", async (req, res) => {
  const { bookingId, examId, valore, note } = req.body as {
    bookingId: number;
    examId: number;
    valore: string;
    note?: string | null;
  ...
  try {
    const existing = await db
      .select()
      .from(refertiTable)
      .where(and(eq(refertiTable.bookingId, bookingId), eq(refertiTable.examId, examId)))
      .limit(1);
    ...
    if (existing[0]) {
      const updated = await db
        ...
        .returning();
      const r = updated[0];
      return res.json({ id: r.id, bookingId: r.bookingId, examId: r.examId, valore: r.valore, note: r.note, refertataAt: r.refertataAt.toISOString() });
    } else {
      const inserted = await db
        .insert(refertiTable)
        .values({ bookingId, examId, valore: valore.trim(), note: note ?? null })
        .returning();
      const r = inserted[0];
      return res.json({ id: r.id, bookingId: r.bookingId, examId: r.examId, valore: r.valore, note: r.note, refertataAt: r.refertataAt.toISOString() });
    }
